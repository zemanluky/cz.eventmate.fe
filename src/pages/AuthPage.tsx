...
export const AuthPage: React.FC = () => {
  ...
  // Function to update hash properly
  const updateHash = (hash: string) => {
    const newHash = `/auth#${hash}`; // Ensure the hash matches your routing expectations
    window.location.hash = newHash; // Updates the URL hash
  };

  React.useEffect(() => {
    const handleHashChange = () => {
      // Update `showLogin` based on the current hash
      setShowLogin(window.location.hash === "#/auth#login");
    };
            ...
            >
              <Flex justifyContent="space-between" alignItems={"center"}>
                ...
                <Button
                  ...
                  color="blue"
                  onClick={() => {
                    const newHash = showLogin ? "register" : "login";
                    updateHash(newHash); // Updates the hash properly
                  }}
                >
